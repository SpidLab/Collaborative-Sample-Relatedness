...
def compute_accuracy():
    ...
    # attacker_df = pd.read_csv("Data/Eye_color/Noisy_data/run_0/noisy_data_mixture_eps_4.0.csv", sep=',', index_col=0)
    attacker_df = original_df.copy()
    ...
    attacker_userIDs = userIDs[0:700]
    ...
    # print(abs(MAF[100:350].diff()).mean())
    MAF = MAF.sort_values()
    # print(abs(MAF[100:350].diff())[1:].mean())
    uniqueSNPlist = []
    for index, value in MAF.items():
        uniqueSNPlist.append(index)
    print(len(uniqueSNPlist))

    # # Suppose that attacker matches first_index with the actual SNP, and tries to find the correlated SNPs
    # first_index = 'rs4778413'
    # # compute the entropy of SNP2 - second_index and sort them in ascending order
    # entropy_per_SNP = {}
    # for SNP in SNPlist:
    #     if SNP != first_index:
    #         dict = original_correlation[(first_index, SNP)]
    #         entropy = 0
    #         for value in dict.values():
    #             if value != 0:
    #                 entropy += math.log2(value) * value
    #         entropy *= -1
    #         entropy_per_SNP[SNP] = entropy
    # sorted_entropy_SNPs = {k: v for k, v in sorted(entropy_per_SNP.items(), key=itemgetter(1))}

    m = 350 # number of snps in attacker dataset
    n = 250 # number of snps in original dataset
    # attacker_SNPlist = random.sample(uniqueSNPlist, m)
    attacker_SNPlist = uniqueSNPlist[100:100 + m]
    original_SNPlist = attacker_SNPlist[0:n]

    attacker_df = attacker_df.loc[attacker_SNPlist]
    original_df = original_df.loc[original_SNPlist]
    ...
    # print(attacker_MAF)
    MAF_difference = {}
    for SNP1 in original_SNPlist:
        for SNP2 in attacker_SNPlist:
            MAF_difference[(SNP1, SNP2)] = abs(attacker_MAF[SNP2] - original_MAF[SNP1])
    ...
    first_index = original_SNPlist[0]
    ...
    for j in range(len(original_SNPlist)):
        if first_index != original_SNPlist[j]:
            second_index = original_SNPlist[j]
    ...
    for j in range(len(attacker_SNPlist)):
        if first_index != attacker_SNPlist[j]:
            second_index = attacker_SNPlist[j]
    ...
    for SNP in original_SNPlist:
        ...
        if first_index != second_index:
            ...
            min_index = "rs..."
            other_diff = 0
            for i in range(len(attacker_SNPlist)):
                if attacker_SNPlist[i] != first_index:#we don't consider distance between the same SNP
                    third_index = attacker_SNPlist[i]
