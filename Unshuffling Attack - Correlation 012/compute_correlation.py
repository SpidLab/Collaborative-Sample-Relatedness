...
userIDs = caseIDs + controlIDs #holds all user IDs
...
second_index = 'rs6606817'
actual_probability_dictionary = correlation[(first_index,second_index)]
...
for i in range(len(SNPlist)):
    if SNPlist[i] != first_index:#we don't consider distance between the same SNP
        ...
        possible_probability_dictionary = correlation[(first_index, third_index)]
        diff = compute_difference(actual_probability_dictionary, possible_probability_dictionary)
        f.write(str(first_index) + "," + str(second_index) + "," + str(third_index) + "," +str(diff)+"\n")
f.close()
