...
new_df = pd.read_csv("Data/Eye_color/original_mixture_chrm15.csv", sep=',', index_col=0)
new_df = new_df.head(200) # select only the top X SNPs, because of performance issues
...
original_userIDs = userIDs[0:900]

original_df = new_df[original_userIDs]
new_df = new_df[userIDs]
...
SNPlist = new_df.index.values.tolist()
for i in range(len(SNPlist)):
    for j in range(len(SNPlist)):
        if i != j:
            first_index = SNPlist[i]
...
for i in range(len(SNPlist)):
    for j in range(len(SNPlist)):
        if i != j:
            first_index = SNPlist[i]
            second_index = SNPlist[j]
            first_list_series = pd.Series(new_df.loc[first_index])
            second_list_series = pd.Series(new_df.loc[second_index])
            first_list = pd.Series(new_df.loc[first_index]).to_numpy()
            second_list = pd.Series(new_df.loc[second_index]).to_numpy()
            new_correlation[(first_index, second_index)] = compute_probabilities(first_list, second_list)

#Testing purposes --------------
# first_index = SNPlist[0]
# second_index = SNPlist[1]
# first_list_series = pd.Series(new_df.loc[first_index])
# second_list_series = pd.Series(new_df.loc[second_index])
# first_list = pd.Series(new_df.loc[first_index]).to_numpy()
# second_list = pd.Series(new_df.loc[second_index]).to_numpy()
# correlation[(first_index, second_index)]= compute_probabilities(first_list, second_list)
#------------------------------
...
count1 = 0
index = 0
for key in sorted_entropy_SNPs:
    second_index = key
    if first_index != second_index:
        ...
        for i in range(len(SNPlist)):
            if SNPlist[i] != first_index:#we don't consider distance between the same SNP
                ...
                possible_probability_dictionary = new_correlation[(first_index, third_index)]
        ...
        if min_index == second_index:
            ...
            print(new_correlation[(first_index, second_index)])
            print(other_diff)
            print(new_correlation[(first_index, min_index)])
            print(min)
            print(index)
            if min != other_diff:
                print("Different")
...
# rs6576313 rs4778531 0.008110403397023956


# first_index = 'rs4778413'
# second_index = 'rs28490428'
# third_index = 'rs28490428'
# actual_probability_dictionary = original_correlation[(first_index, second_index)]
# possible_probability_dictionary = new_correlation[(first_index, third_index)]
# diff = compute_difference(actual_probability_dictionary, possible_probability_dictionary)
# print(diff)
